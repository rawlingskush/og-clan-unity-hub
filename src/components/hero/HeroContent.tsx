      ...
      <AnimatedContent animation="fade-in-up" delay={300}>
        <h1 className={`text-4xl sm:text-5xl md:text-7xl font-bold tracking-tight mb-2 md:mb-3 relative ${isMobile ? 'px-3' : ''}`}>
          <span className="relative inline-block overflow-hidden group">
            ...
            {/* Glitch effect layer */}
            <span className="absolute inset-0 bg-clip-text text-transparent bg-gradient-to-r from-red-500/50 via-ogclan/50 to-blue-500/50 blur-[3px] animate-pulse opacity-0 group-hover:opacity-70 transition-opacity duration-300"></span>
            ...
            {/* Tech highlight flare */}
            <span className="absolute inset-0 opacity-0 group-hover:opacity-100 bg-gradient-to-r from-ogclan-dark/0 via-ogclan/20 to-ogclan-light/0 transition-all duration-500"></span>
            
            {/* Digital circuit pattern overlay */}
            <span className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-30 mix-blend-overlay group-hover:opacity-60 transition-opacity duration-700"></span>
          </span>
          
          {/* High-tech background glow */}
          <span className="absolute -inset-1 bg-gradient-to-r from-blue-500/10 via-ogclan/30 to-red-500/10 blur-xl opacity-50 animate-pulse-slow -z-10"></span>
        </h1>
        ...
        <p className="text-xl sm:text-2xl md:text-3xl text-ogclan-light font-semibold flex items-center justify-center gap-2 relative">
          ...
          {/* Tech accent */}
          <span className="absolute -right-4 top-1/2 h-[4px] w-[4px] rounded-full bg-red-500 shadow-[0_0_8px_red] animate-pulse-slow"></span>
          <span className="absolute -left-4 top-1/2 h-[4px] w-[4px] rounded-full bg-blue-500 shadow-[0_0_8px_blue] animate-pulse-slow animation-delay-600"></span>
        </p>
      </AnimatedContent>

      <AnimatedContent animation="fade-in-up" delay={600}>
        <p className="mt-3 md:mt-5 text-base sm:text-lg md:text-xl max-w-6xl mx-auto text-balance leading-relaxed px-4 sm:px-2 text-gray-400 relative">
